#!/bin/sh
# Tags origin/main as v<version from fabric/fabric.mod.json> and pushes the tag, which
...
git ls-remote --exit-code --tags origin "refs/tags/$tag" >/dev/null &&
  { echo "$tag already exists -- bump version in fabric/fabric.mod.json" >&2; exit 1; }
